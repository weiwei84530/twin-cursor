...
- Click the small **colour chip** next to it to colour that mouse's cursor. The popup offers a twelve-colour palette, a **Custom…** picker and **System default** (no tint). The tint keeps the cursor's shape and outline and only replaces its hue. The real cursor is coloured with `SetSystemCursor`, which is a system-wide change: the arrow, I-beam, hand, resize and "no" cursors are replaced while TwinCursor runs and restored when it exits (the animated busy cursors are left alone). It is rendered at the primary monitor's cursor size, so on a mixed-DPI setup the real cursor keeps that size everywhere. If TwinCursor is killed instead of exiting normally, changing the pointer scheme in Windows' mouse settings restores the cursors.
