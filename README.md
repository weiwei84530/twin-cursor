...
- Click the small **colour chip** next to it to colour that mouse's cursor. The popup offers a twelve-colour palette, a **Custom…** picker and **System default** (no tint). Colours mixed in the picker come back as a **Recent** row under the palette, one palette width of them (six), newest first, so a colour that is not on the palette can be picked again without mixing it a second time. The tint keeps the cursor's shape and outline and only replaces its hue. The real cursor is coloured with `SetSystemCursor`, which is a system-wide change: the arrow, hand, resize and "no" cursors are replaced while TwinCursor runs and restored when it exits. The animated busy cursors are left alone, and so are the text I-beam and the crosshair: those invert the pixels behind them instead of painting a colour, which is what keeps them readable on any background, and a coloured copy would lose that. It is rendered at the primary monitor's cursor size, so on a mixed-DPI setup the real cursor keeps that size everywhere. If TwinCursor is killed instead of exiting normally, changing the pointer scheme in Windows' mouse settings restores the cursors.
...
- **Restore Defaults** puts everything back to a fresh install: mirroring off, cursor colours and the recent-colour list cleared, the first slot's hotkey back to `Ctrl+Alt+M` and the second slot's disabled, both slots filled automatically again, and **Start with Windows** turned off.
